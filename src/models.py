...
class SchoolClass(models.Model):
    ...
    section = models.ForeignKey(Section, on_delete=models.CASCADE, related_name="classes", null=True, blank=True)
...
class FeeStructure(models.Model):
    ...
    term_group = models.CharField(
        max_length=20,
        choices=TERM_GROUP_CHOICES
    )
    ...
    transport = models.BooleanField(
        default=False,
        help_text="Includes transportation fee"
    )
    ...
    def __str__(self):
        return (
            f"{self.section} | "
            f"{self.get_term_group_display()} | "
            f"{self.get_student_type_display()} | "
            f"{'With Transport' if self.transport else 'No Transport'}"
        )
...
class FeeComponent(models.Model):
    ...
    name = models.CharField(
        max_length=50,
        help_text="e.g. Tuition, Feeding, Transport, Uniform"
    )
...
class MidTermResult(models.Model):
    ...
    def __str__(self):
        return f"Mid Term Score {self.student} - {self.class_assigned} ({self.term.name} {self.session.name} {self.subject.name})"
...
class Payment(models.Model):
    student = models.ForeignKey(Student, on_delete=models.CASCADE)
    transaction_reference = models.CharField(max_length=50, null=True, blank=True, unique=True)
...
class PartPaymentApproval(models.Model):
    ...
    approved_amount = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        help_text="Maximum amount allowed for part payment"
    )
...
class FeeWaiverApproval(models.Model):
    ...
    waiver_percentage = models.PositiveIntegerField(
    ...
    def __str__(self):
        return (
            f"{self.student} | {self.session} | {self.term} | "
            f"{self.waiver_percentage}% | {self.status}"
        )
